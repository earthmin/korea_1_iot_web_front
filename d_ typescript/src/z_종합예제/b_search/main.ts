{
  ...
  const filterUsers = (users: Users, query: string): Users => {
    return users.filter(
      (user) =>
        ...
        user.username.toLowerCase().includes(query.toLowerCase()) || // or연산자
  ...
  const sortUsers = (users: Users, key: "name" | "email"): Users => {
    // 큰 따옴표 안에 있는 문자열(리터럴타입) 그대로만 가져올수 있다 (userName은 불가) 
    // | 는 or인데 유니온 타입이당 
    ...
    //  key가 name으로 전달된 경우 a[name].localComparr(b[name])
