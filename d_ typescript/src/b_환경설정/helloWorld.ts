...
// : ts 코드는 실시간으로 js로 컴파일 X

//! ts-node
// : ts 코드를 실시간으로 js로 컴파일 하고 실행 (js 파일 생성 X)

//? ts-node 설치 
// : node-modules 폴더가 존재하는 프로젝트 최상단의 터미널에서 설치 
// >> npm install -g ts-node
// >> npm install ts-node --save-dev 

console.log('ts-node로 ts파일 실행하지');
// ts-node 파일명.ts
...
// >> 동일한 이름의 js 파일이 생성 
// +) tsc로 컴파일 된 js 파일에서도 실시간 컴파일 이루어지지 X
